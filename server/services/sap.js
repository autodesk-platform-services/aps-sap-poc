...
function getFacilities() {
    return new Promise(function (resolve, reject) {
        ...
        const stmt = client.prepare(`SELECT * FROM ${SAP_HANA_SPACE}."Bike_Shop_Facility_Overview"`);
...
function getStockOverview() {
    return new Promise(function (resolve, reject) {
        const client = hana.createConnection();
        client.connect(options);
        const stmt = client.prepare(`SELECT * FROM ${SAP_HANA_SPACE}."Bike_Shop_Stock_Overview"`);
        stmt.exec([], function (err, results) {
            stmt.drop();
            client.disconnect();
            if (err) {
...
function getArticleDetail(partNumber) {
    return new Promise(function (resolve, reject) {
        const client = hana.createConnection();
        client.connect(options);
        const stmt = client.prepare(`SELECT * FROM ${SAP_HANA_SPACE}."Bike_Shop_Article_Detail" (?)`);
        stmt.exec([partNumber], function (err, results) {
            stmt.drop();
            client.disconnect();
            if (err) {
                reject(err);
...
function getDeliveryOptions() {
    return new Promise(function (resolve, reject) {
        const client = hana.createConnection();
        client.connect(options);
        const stmt = client.prepare(`SELECT * FROM ${SAP_HANA_SPACE}."Delivery_Options_CO2_Price"`);
        stmt.exec([], function (err, results) {
            stmt.drop();
            client.disconnect();
            if (err) {
